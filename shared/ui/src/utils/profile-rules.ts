...
export function isDefaultUserDir(b: BCPBrowser, p: BCPProfile): boolean {
  // 主程序托管型浏览器的默认路径就是可用的主程序配置（店铺/环境窗口以 children 展示）
  if (managesOwnProfiles(b.browser_type)) return false;
  return !!b.default_user_data_dir && p.user_data_dir === b.default_user_data_dir;
}

/**
 * 主程序入口 profile（由业务侧扩展通过 isMainEntry 判定）。
 * 它不是可控环境，不能作为自动化控制目标：在「当前浏览器配置」中排第一位、
 * 锁定不可勾选，右键「打开」直接打开其主程序。
 */
export function isMainEntryProfile(b: BCPBrowser, p: BCPProfile): boolean {
  return getBrowserUIExtension(b.browser_type)?.isMainEntry?.(p.id) === true;
}
...
export function getSortGroup(b: BCPBrowser, p: BCPProfile, counts?: Record<string, number>): number {
  if (isMainEntryProfile(b, p)) return 0;
