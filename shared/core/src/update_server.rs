...
//!   2. 编译时 OS env（build.rs 注入；fallback 默认 = 公司开发机 LAN IP 192.168.10.28:8080）
//!
//! @hard-rules
//! - 本模块是「公司版本服务器 IP 192.168.3.51」字面值的**唯一替换入口**。
//! - 公司开发机（2026-09-23 ipconfig 实测 IP=192.168.10.28）当前作为发布服务器使用；
//!   build.rs fallback 默认指向此 IP。
//! - 严禁加入 fallback 到 127.0.0.1 / localhost / "调用者自己 IP" 的设计 ——
//!   同事电脑跑 release 时不能把"自己的电脑"误当成版本服务器；release 嵌入的 URL
//!   必须是**预先指定的一台固定机器**（参见 frame-update-2026-09-23）。
//!
//! @接口清单
